...
export function apply(ctx) {
  ...
  const registerWeb = () => {
    ...
    route('/plugins/file-explorer/write', async (req, res) => {
      ...
      try {
        const target = await fs.resolve(path)
        // The file explorer is a user-facing editor, not an agent tool: it must
        // be able to write anywhere the user can, matching the session's own
        // danger-full-access policy instead of the workspace-write default.
        // This mirrors the launcher's "the user owns this machine" stance.
        await fs.writeText(target, String((body && body.content) ?? ''), undefined, undefined, { mode: 'danger-full-access' })
