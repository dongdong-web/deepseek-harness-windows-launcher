...
`;
		...
		const ExplorerPanel = (props) => {
			...
			const writeFile = (path, content) => api.write(path, content).then((res) => {
				if (res && res.error) {
					showStatus({ ok: false, text: '保存失败：' + res.error });
					return false;
				}
				showStatus({ ok: true, text: '已自动保存' });
				return true;
			}).catch((err) => {
				showStatus({ ok: false, text: '保存失败：' + String((err && err.message) || err) });
				return false;
			});
			...
			const onSave = () => {
				...
				writeFile(path, content).then((ok) => {
					if (ok) setEditor((e) => e && e.path === path ? { ...e, editing: false } : e);
				});
			};
			...
			const scheduleAutoSave = () => {
				...
				saveTimerRef.current = setTimeout(() => {
					saveTimerRef.current = null;
					if (!editor || editor.state !== 'ready' || !editor.editing) return;
			...
			const renderTree = () => {
				...
				if (tree.expanded.has(tree.rootPath)) {
					...
					if (children) {
						for (const child of children) {
							rows.push(react.createElement(TreeNode, { key: child.path, entry: child, depth: 1, tree, onToggle: toggleDir, onSelect: selectFile, onOpen: (e) => openFile(e, true) }));
						}
			...
			const renderSearch = () => {
				...
				for (const m of s.matches) {
					...
					rows.push(react.createElement('div', {
						...
						style: { paddingLeft: 6 },
						onClick: () => m.type === 'directory' ? selectFile(m.path) : openFile(m, true),
						onDoubleClick: () => m.type === 'directory' ? selectFile(m.path) : openFile(m, true),
						title: m.path,
					},
			...
			const renderPreview = () => {
				...
				const head = react.createElement('div', { className: 'fe-editor-head' },
					...
					editor.state === 'ready' && editor.editing
						...
						: null,
					react.createElement('button', { className: 'fe-btn', onClick: () => { flushSaveTimer(); setEditor(null); setStatus(null) } }, '关闭'),
				);
				let body = null;
