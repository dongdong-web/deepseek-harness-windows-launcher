...
8. 发布包为 ZIP，只打入明确白名单中的运行时、应用、启动器、运行时清单和发布文档；同时生成 SHA-256 文件供下载者核验。它不使用自解压 EXE，也不写入安装目录或创建快捷方式。
9. 工作区选择固定使用官方 Harness 的网页内 `browse` 后端。启动器通过随包分发的 `--patch` 覆盖层装配该后端，不修改用户的 DSH profile，也不触发可能停留在任务栏后台的 Windows 原生文件夹窗口。
