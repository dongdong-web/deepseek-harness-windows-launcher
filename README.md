...
**好消息:更新不用卸载、不用删除数据,直接覆盖解压即可。**

- 你的**所有数据**(会话、凭据、工作区、配置)都存在 `%LOCALAPPDATA%\DeepSeekHarness`,**不在**解压目录里
- 更新时只需替换程序文件,数据原封不动

### 更新步骤(3 步)

1. **先退出正在运行的 DeepSeek Harness**(重要!否则文件被占用,覆盖会失败):
   - 网页侧边栏底部 → "退出 DeepSeek Harness",或
   - 双击 `launcher\start.cmd` 后输入 `stop`
2. **下载新版本 ZIP**,解压到**原来的目录**(选择"全部覆盖/替换"),或者解压到新目录后删掉旧的也行
3. **双击 `launcher\start.vbs`** 重新启动

启动器会自动完成所有收尾工作:重建内部链接、刷新桌面快捷方式、同步插件配置。**你的历史会话、API Key、工作区文件都不会丢。**

> 💡 如果启动器提示"文件被占用/无法替换",多半是第 1 步没做彻底——彻底退出后再覆盖即可。
...
**Q: 出了新版本怎么更新?会丢数据吗?**
不会丢数据。见上方 [🔄 更新到新版本](#-更新到新版本):先退出 → 下载新 ZIP 覆盖解压到原目录 → 重新启动。数据(会话/密钥/工作区)都存在 `%LOCALAPPDATA%\DeepSeekHarness`,更新程序不影响。
