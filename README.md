...
启动器会处理端口冲突、单实例、日志、浏览器打开、私有环境变量和进程退出。侧边栏底部提供“退出 DeepSeek Harness”按钮，确认后会优雅关闭当前后台服务，随后可直接关闭浏览器标签页。为避免 Windows 原生文件夹窗口被放到任务栏后台，工作区选择默认使用官方 Harness 提供的网页内目录浏览器；不会启动额外的系统选择窗口。启动器还会在该界面中可视化列出 Windows 已挂载的盘符，点击后可继续浏览其下的文件夹；网络共享仍可在“输入路径”中填写 UNC 路径。
